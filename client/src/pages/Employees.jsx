...
const Employees = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (isEditing) {
                const response = await updateEmployee(currentEmployee.id, submissionData);
    ...
    const confirmDelete = async () => {
        ...
        try {
            if (projectIdFilter) {
                if (!offboardDate) {
                    toast.error("Please select an offboard date.");
                    setIsDeleting(false);
                    return;
                }

                // We assume start_date is not strictly validated here against the API response,
                // but we pass offboardDate to the backend which now handles it safely
                await offboardProjectResource(projectIdFilter, employeeToDelete.id, offboardDate);
                toast.success('Employee off-boarded from project');
            } else {
    ...
    return (
        <div>
                            ...
                            >
                                <div className="p-4 flex items-center gap-3">
                                    ...
                                    <div className="flex-1 min-w-0">
                                        ...
                                        <p className="text-xs text-slate-500 dark:text-slate-400 truncate mt-0.5">{employee.role}</p>
                                        <div className="flex items-center gap-2 mt-2">
                                            {employee.offboarded_date ? (
                                                <span className="flex-shrink-0 inline-flex items-center px-1.5 py-0.5 rounded text-[9px] font-bold uppercase tracking-wider bg-slate-100 text-slate-500 dark:bg-slate-700 dark:text-slate-400">
                                                    Offboarded
                                                </span>
                                            ) : (
                                                <span className={cn(
                                ...
                                {!employee.offboarded_date && (
                                    <div className="absolute top-2 right-2 flex gap-1 transform translate-x-12 opacity-0 group-hover:translate-x-0 group-hover:opacity-100 transition-all duration-200">
                                        ...
                                        <button
                                            ...
                                            title={projectIdFilter ? "Off-board Resource" : "Remove Employee"}
                                        >
            ...
            <Modal
                ...
                title={projectIdFilter ? "Off-board Resource" : "Delete Employee"}
            >
                <div>
                    ...
                    <p className="text-slate-600 dark:text-slate-300 mb-6">
                        {projectIdFilter ? (
                            <>
                                <span className="block mb-4">Are you sure you want to off-board <span className="font-bold text-slate-900 dark:text-white">{employeeToDelete?.name}</span> from this project? Their recorded costs will be preserved up to today.</span>
                                ...
                                <input
                                    type="date"
                                    className="w-full px-4 py-3 border-2 border-primary rounded-lg focus:outline-none focus:ring-4 focus:ring-primary/30 bg-white dark:bg-slate-800 text-slate-900 dark:text-white dark:[color-scheme:dark] shadow-sm font-medium"
                                    value={offboardDate}
                                    onChange={(e) => setOffboardDate(e.target.value)}
                                />
                            </>
                        ) : (
                            <>Are you sure you want to delete <span className="font-bold text-slate-900 dark:text-white">{employeeToDelete?.name}</span>? This will check if any resource plans are linked (cascade delete is enabled).</>
                        )}
                    </p>
